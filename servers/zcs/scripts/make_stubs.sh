...
# If the zcs_core dependency has been updated, then this script should be run
...
echo "Update the ZCS.rdl to define string type"
RDL_FILE=src/main/rdl/ZCS.rdl

echo "Generate the ZCS server stubs"
rdl -s generate -b="/v1" -o="src/main/java" java-server $RDL_FILE

echo "Removing not needed ZCS Server file..."
rm src/main/java/com/yahoo/athenz/zcs/ZCSServer.java
